...
(deftest apply-on-input-coercion
  (let [i-client (client/wrap-input-coercion identity)
        resp-str (i-client {:body "foo"})
        resp-bytes (i-client {:body (.getBytes "foo")})]
    (is (= "UTF-8" (:character-encoding resp-str)))
    (is (= "foo" (slurp (:body resp-str))))
    (is (= "foo" (slurp (:body resp-bytes))))))
